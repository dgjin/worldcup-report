...
// api/wc/gallery/likes.ts
async function sbFetch(env2, path, options = {}) {
  const base = env2.SUPABASE_URL;
...
var onRequest = /* @__PURE__ */ __name(async (ctx) => {
  ...
  try {
    if (ctx.request.method === "GET") {
      ...
      const map = {};
      for (const r of rows ?? []) {
        if (r.photo_key === "__app__") continue;
    ...
    if (ctx.request.method === "POST") {
      ...
      await sbFetch(sb, existing?.length ? `gallery_likes?photo_key=eq.${encodeURIComponent(pk)}` : "gallery_likes", {
        ...
        body: JSON.stringify(
          existing?.length ? { likes: currentLikes + 1 } : { photo_key: pk, likes: 1 }
        ),
        headers: existing?.length ? {} : { "Prefer": "resolution=ignore-duplicates" }
      });
    ...
    console.error("[gallery-likes]", e.message);
    if (ctx.request.method === "GET") {
...
// api/wc/gallery/refresh.ts
var ABC_GALLERY_URL = "https://abcnews.go.com/Sports/photos/best-photos-fifa-world-cup-2026-133075564";
var USATODAY_GALLERY_URL = "https://www.usatoday.com/picture-gallery/sports/soccer/worldcup/2026/06/13/world-cup-2026-best-photos/90528304007/";
var APNEWS_GALLERY_URLS = [
...
async function collectAbcNews() {
  ...
  const sourceMap = { gty: "Getty Images", rt: "Reuters", ap: "Associated Press" };
...
__name(collectAbcNews, "collectAbcNews");
...
__name(collectUsaToday, "collectUsaToday");
async function collectApNews() {
  ...
  for (const galleryUrl of APNEWS_GALLERY_URLS) {
    ...
    for (const m of html.matchAll(dimsRE)) {
      ...
      if (!seen2.has(hash)) {
        seen2.add(hash);
        assetsUrls.push(`https://assets.apnews.com/${hash}`);
      }
    }
...
__name(collectApNews, "collectApNews");
var onRequest2 = /* @__PURE__ */ __name(async (ctx) => {
  ...
  try {
    ...
    try {
      ...
      results.abcnews = -1;
      console.error("[refresh:abc]", e.message);
    }
    try {
      results.usatoday = (await collectUsaToday()).length;
    } catch (e) {
      results.usatoday = -1;
      console.error("[refresh:usa]", e.message);
    }
    try {
      results.apnews = (await collectApNews()).length;
    } catch (e) {
      results.apnews = -1;
      console.error("[refresh:ap]", e.message);
    }
    ...
    if (kv) {
      try {
        const [abcPhotos, usaPhotos, apPhotos] = await Promise.all([
          collectAbcNews().catch(() => []),
          collectUsaToday().catch(() => []),
          collectApNews().catch(() => [])
        ]);
        ...
        console.error("[refresh:kv]", e.message);
      }
    }
    ...
    console.error("[gallery-refresh]", e.message);
    return new Response(JSON.stringify({
      ok: false,
      error: e.message
    }), { status: 500, headers });
...
// api/app/likes.ts
async function sbFetch2(env2, path, options = {}) {
  const base = env2.SUPABASE_URL;
  const key = env2.SUPABASE_SERVICE_ROLE_KEY;
  if (!base || !key) throw new Error("Supabase not configured");
  const res = await fetch(`${base}/rest/v1/${path}`, {
...
var onRequest3 = /* @__PURE__ */ __name(async (ctx) => {
  ...
  try {
    if (ctx.request.method === "GET") {
      const rows = await sbFetch2(sb, `gallery_likes?select=likes&photo_key=eq.${KEY}`);
      const likes = rows?.[0]?.likes ?? 0;
      return new Response(JSON.stringify({ likes }), { headers });
    }
    if (ctx.request.method === "POST") {
      const existing = await sbFetch2(sb, `gallery_likes?select=likes&photo_key=eq.${KEY}`);
      const currentLikes = existing?.[0]?.likes ?? 0;
      if (existing?.length) {
        await sbFetch2(sb, `gallery_likes?photo_key=eq.${KEY}`, {
          method: "PATCH",
          body: JSON.stringify({ likes: currentLikes + 1, updated_at: (/* @__PURE__ */ new Date()).toISOString() })
        });
      } else {
        await sbFetch2(sb, "gallery_likes", {
          method: "POST",
          headers: { Prefer: "resolution=ignore-duplicates" },
          body: JSON.stringify({ photo_key: KEY, likes: 1, updated_at: (/* @__PURE__ */ new Date()).toISOString() })
        });
      }
      headers["Cache-Control"] = "no-store";
    ...
    console.error("[app-likes]", e.message);
    if (ctx.request.method === "GET") {
...
// api/app/vote.ts
var CATEGORIES = ["champion", "runnerup", "thirdplace"];
async function sbFetch3(env2, path, options = {}) {
  const base = env2.SUPABASE_URL;
  const key = env2.SUPABASE_SERVICE_ROLE_KEY;
  if (!base || !key) throw new Error("Supabase not configured");
  const res = await fetch(`${base}/rest/v1/${path}`, {
...
async function readAggregate(env2) {
  const result = {
    champion: {},
    runnerup: {},
    thirdplace: {}
  ...
  try {
    const rows = await sbFetch3(env2, `gallery_likes?select=photo_key,likes&photo_key=like.vote_*`);
...
__name(readAggregate, "readAggregate");
async function countVoteRecords(env2) {
  try {
    const rows = await sbFetch3(env2, `wc_data?select=type&type=like.vote_record_*`);
    return rows?.length ?? 0;
  } catch {
    return 0;
  }
}
__name(countVoteRecords, "countVoteRecords");
async function readAllRecords(env2) {
  try {
    const rows = await sbFetch3(env2, `wc_data?select=data&type=like.vote_record_*`);
    return (rows ?? []).map((r) => r.data).filter(Boolean).sort((a, b) => String(b?.ts ?? "").localeCompare(String(a?.ts ?? "")));
  } catch {
    return [];
  }
}
__name(readAllRecords, "readAllRecords");
async function findVoteByEmail(env2, email) {
  try {
    const rows = await sbFetch3(env2, `wc_data?select=data&type=like.vote_record_*`);
    for (const row of rows ?? []) {
...
__name(findVoteByEmail, "findVoteByEmail");
function genId() {
...
var onRequest4 = /* @__PURE__ */ __name(async (ctx) => {
  ...
  try {
    if (ctx.request.method === "GET") {
      ...
      const result = { ...aggregate, total, voters };
      if (email) {
    ...
    if (ctx.request.method === "POST") {
      ...
      try {
        ...
        console.error("[vote] record save failed:", e.message);
      }
      for (const { cat, team } of picks) {
        const key = `vote_${cat}_${team}`;
        try {
          const existing = await sbFetch3(sb, `gallery_likes?select=likes&photo_key=eq.${encodeURIComponent(key)}`);
          const current = existing?.[0]?.likes ?? 0;
          if (existing?.length) {
            await sbFetch3(sb, `gallery_likes?photo_key=eq.${encodeURIComponent(key)}`, {
              method: "PATCH",
              body: JSON.stringify({ likes: current + 1, updated_at: (/* @__PURE__ */ new Date()).toISOString() })
            });
          } else {
            await sbFetch3(sb, "gallery_likes", {
              method: "POST",
              headers: { Prefer: "resolution=ignore-duplicates" },
              body: JSON.stringify({ photo_key: key, likes: 1, updated_at: (/* @__PURE__ */ new Date()).toISOString() })
            });
...
// api/wc/gallery/index.ts
var ABC_GALLERY_URL2 = "https://abcnews.go.com/Sports/photos/best-photos-fifa-world-cup-2026-133075564";
var USATODAY_GALLERY_URL2 = "https://www.usatoday.com/picture-gallery/sports/soccer/worldcup/2026/06/13/world-cup-2026-best-photos/90528304007/";
var APNEWS_GALLERY_URLS2 = [
  "https://apnews.com/photo-gallery/photos-soccer-world-cup-shakira-opening-ceremony-608e920d1bf477e7aa544fd2b139331e",
  "https://apnews.com/photo-gallery/photos-brazil-morocco-haiti-metlife-qatar-world-cup-ba66730f4a4b4e341942397a6d512c8c",
  "https://apnews.com/photo-gallery/world-cup-photos-soccer-5dee70b837032094e659a0f0a13a8dfe",
  "https://apnews.com/photo-gallery/world-cup-photos-soccer-usmnt-australia-brazil-haiti-4eb587ca785841b2a1328d8b894faf88",
...
var KV_TTL_MS = 24 * 60 * 60 * 1e3;
async function fetchFromKV(kv) {
  try {
...
__name(fetchFromKV, "fetchFromKV");
async function bgRefreshKV(kv) {
  try {
    const [abcPhotos, usaPhotos, apPhotos] = await Promise.all([
    ...
    if (allPhotos.length > 0) {
      ...
      await kv.put("latest", JSON.stringify({ photos: unique, collectedAt: (/* @__PURE__ */ new Date()).toISOString() }));
      console.log(`[gallery] KV \u540E\u53F0\u5237\u65B0\u6210\u529F\uFF0C${unique.length} \u5F20\u7167\u7247\uFF08ABC+USA+AP\uFF09`);
    }
  } catch (e) {
    console.error("[gallery] KV \u540E\u53F0\u5237\u65B0\u5931\u8D25:", e.message);
  }
}
__name(bgRefreshKV, "bgRefreshKV");
async function fetchNewsApi(key, page) {
...
__name(fetchNewsApi, "fetchNewsApi");
async function fetchAbcNews() {
  try {
    const r = await fetch(ABC_GALLERY_URL2);
    if (!r.ok) return null;
    const html = await r.text();
    const imgRE = /https?:\/\/i\.abcnewsfe\.com\/a\/[a-f0-9-]+\/(wc-[\w.-]+)/gi;
    const seen = /* @__PURE__ */ new Set();
...
__name(fetchAbcNews, "fetchAbcNews");
async function fetchUsaToday() {
  try {
    const r = await fetch(USATODAY_GALLERY_URL2, {
...
__name(fetchUsaToday, "fetchUsaToday");
async function fetchApNews() {
  const allPhotos = [];
  for (const galleryUrl of APNEWS_GALLERY_URLS2) {
    try {
...
__name(fetchApNews, "fetchApNews");
var onRequest5 = /* @__PURE__ */ __name(async (ctx) => {
  ...
  if (kv) {
    ...
    if (cached) {
      ...
      return new Response(JSON.stringify({
        photos: slice,
        next_page: start + PER_PAGE < cached.photos.length ? String(page + 1) : void 0,
        source: "abcnews",
  ...
  if (newsKey) {
    ...
    if (photos) {
      return new Response(JSON.stringify({
        photos,
        next_page: photos.length >= PER_PAGE ? String(page + 1) : void 0,
        source: "newsapi"
      }), {
        headers: {
          ...headers,
  ...
  if (abcPhotos) {
    ...
    return new Response(JSON.stringify({
      photos: slice,
      next_page: start + PER_PAGE < abcPhotos.length ? String(page + 1) : void 0,
      source: "abcnews"
    }), {
      headers: {
        ...headers,
  ...
  if (usaPhotos) {
    ...
    return new Response(JSON.stringify({
      photos: slice,
      next_page: start + PER_PAGE < usaPhotos.length ? String(page + 1) : void 0,
      source: "usatoday"
    }), {
      headers: {
        ...headers,
  ...
  if (apPhotos) {
    ...
    return new Response(JSON.stringify({
      photos: slice,
      next_page: start + PER_PAGE < apPhotos.length ? String(page + 1) : void 0,
      source: "apnews"
    }), {
      headers: {
        ...headers,
...
// ../data/snapshot.json
var snapshot_default = { _meta: { source: "snapshot", asOf: "2026-06-24", note: "\u8D5B\u679C\u4E0E\u79EF\u5206\u4E3A\u771F\u5B9E\u6570\u636E\uFF08\u622A\u81F3 2026-06-23\uFF0C\u4EA4\u53C9\u9A8C\u8BC1\u81EA NBC/ESPN\uFF09\uFF1B\u5C04\u624B\u699C\u4E3A\u793A\u4F8B\u6570\u636E\uFF0C\u771F\xB7\u5B9E\u65F6\u8BF7\u914D\u7F6E FOOTBALL_DATA_TOKEN\u3002" }, competition: { id: 2e3, code: "WC", name: "FIFA World Cup", type: "CUP", emblem: "https://crests.football-data.org/qatar.png" }, standings: { competition: { id: 2e3, code: "WC", name: "FIFA World Cup" }, season: { id: 2026, startDate: "2026-06-11", endDate: "2026-07-19", currentMatchday: 3 }, standings: [{ stage: "GROUP_STAGE", type: "TOTAL", group: "GROUP_A", table: [{ position: 1, team: { id: 1, name: "Mexico", tla: "MEX" }, playedGames: 2, form: null, won: 2, draw: 0, lost: 0, points: 6, goalsFor: 3, goalsAgainst: 0, goalDifference: 3 }, { position: 2, team: { id: 2, name: "South Korea", tla: "KOR" }, playedGames: 2, form: null, won: 1, draw: 0, lost: 1, points: 3, goalsFor: 2, goalsAgainst: 2, goalDifference: 0 }, { position: 3, team: { id: 3, name: "Czechia", tla: "CZE" }, playedGames: 2, form: null, won: 0, draw: 1, lost: 1, points: 1, goalsFor: 2, goalsAgainst: 3, goalDifference: -1 }, { position: 4, team: { id: 4, name: "South Africa", tla: "RSA" }, playedGames: 2, form: null, won: 0, draw: 1, lost: 1, points: 1, goalsFor: 1, goalsAgainst: 3, goalDifference: -2 }] }, { stage: "GROUP_STAGE", type: "TOTAL", group: "GROUP_B", table: [{ position: 1, team: { id: 5, name: "Canada", tla: "CAN" }, playedGames: 2, form: null, won: 1, draw: 1, lost: 0, points: 4, goalsFor: 7, goalsAgainst: 1, goalDifference: 6 }, { position: 2, team: { id: 6, name: "Switzerland", tla: "SUI" }, playedGames: 2, form: null, won: 1, draw: 1, lost: 0, points: 4, goalsFor: 5, goalsAgainst: 2, goalDifference: 3 }, { position: 3, team: { id: 7, name: "Bosnia and Herzegovina", tla: "BIH" }, playedGames: 2, form: null, won: 0, draw: 1, lost: 1, points: 1, goalsFor: 2, goalsAgainst: 5, goalDifference: -3 }, { position: 4, team: { id: 8, name: "Qatar", tla: "QAT" }, playedGames: 2, form: null, won: 0, draw: 1, lost: 1, points: 1, goalsFor: 1, goalsAgainst: 7, goalDifference: -6 }] }, { stage: "GROUP_STAGE", type: "TOTAL", group: "GROUP_C", table: [{ position: 1, team: { id: 9, name: "Brazil", tla: "BRA" }, playedGames: 2, form: null, won: 1, draw: 1, lost: 0, points: 4, goalsFor: 4, goalsAgainst: 1, goalDifference: 3 }, { position: 2, team: { id: 10, name: "Morocco", tla: "MAR" }, playedGames: 2, form: null, won: 1, draw: 1, lost: 0, points: 4, goalsFor: 2, goalsAgainst: 1, goalDifference: 1 }, { position: 3, team: { id: 11, name: "Scotland", tla: "SCO" }, playedGames: 2, form: null, won: 1, draw: 0, lost: 1, points: 3, goalsFor: 1, goalsAgainst: 1, goalDifference: 0 }, { position: 4, team: { id: 12, name: "Haiti", tla: "HAI" }, playedGames: 2, form: null, won: 0, draw: 0, lost: 2, points: 0, goalsFor: 0, goalsAgainst: 4, goalDifference: -4 }] }, { stage: "GROUP_STAGE", type: "TOTAL", group: "GROUP_D", table: [{ position: 1, team: { id: 13, name: "United States", tla: "USA" }, playedGames: 2, form: null, won: 2, draw: 0, lost: 0, points: 6, goalsFor: 6, goalsAgainst: 1, goalDifference: 5 }, { position: 2, team: { id: 14, name: "Australia", tla: "AUS" }, playedGames: 2, form: null, won: 1, draw: 0, lost: 1, points: 3, goalsFor: 2, goalsAgainst: 2, goalDifference: 0 }, { position: 3, team: { id: 15, name: "Paraguay", tla: "PAR" }, playedGames: 2, form: null, won: 1, draw: 0, lost: 1, points: 3, goalsFor: 2, goalsAgainst: 4, goalDifference: -2 }, { position: 4, team: { id: 16, name: "T\xFCrkiye", tla: "TUR" }, playedGames: 2, form: null, won: 0, draw: 0, lost: 2, points: 0, goalsFor: 0, goalsAgainst: 3, goalDifference: -3 }] }, { stage: "GROUP_STAGE", type: "TOTAL", group: "GROUP_E", table: [{ position: 1, team: { id: 17, name: "Germany", tla: "GER" }, playedGames: 2, form: null, won: 2, draw: 0, lost: 0, points: 6, goalsFor: 9, goalsAgainst: 2, goalDifference: 7 }, { position: 2, team: { id: 18, name: "Ivory Coast", tla: "CIV" }, playedGames: 2, form: null, won: 1, draw: 0, lost: 1, points: 3, goalsFor: 2, goalsAgainst: 2, goalDifference: 0 }, { position: 3, team: { id: 19, name: "Ecuador", tla: "ECU" }, playedGames: 2, form: null, won: 0, draw: 1, lost: 1, points: 1, goalsFor: 0, goalsAgainst: 1, goalDifference: -1 }, { position: 4, team: { id: 20, name: "Cura\xE7ao", tla: "CUW" }, playedGames: 2, form: null, won: 0, draw: 1, lost: 1, points: 1, goalsFor: 1, goalsAgainst: 7, goalDifference: -6 }] }, { stage: "GROUP_STAGE", type: "TOTAL", group: "GROUP_F", table: [{ position: 1, team: { id: 21, name: "Netherlands", tla: "NED" }, playedGames: 2, form: null, won: 1, draw: 1, lost: 0, points: 4, goalsFor: 7, goalsAgainst: 3, goalDifference: 4 }, { position: 2, team: { id: 22, name: "Japan", tla: "JPN" }, playedGames: 2, form: null, won: 1, draw: 1, lost: 0, points: 4, goalsFor: 6, goalsAgainst: 2, goalDifference: 4 }, { position: 3, team: { id: 23, name: "Sweden", tla: "SWE" }, playedGames: 2, form: null, won: 1, draw: 0, lost: 1, points: 3, goalsFor: 6, goalsAgainst: 6, goalDifference: 0 }, { position: 4, team: { id: 24, name: "Tunisia", tla: "TUN" }, playedGames: 2, form: null, won: 0, draw: 0, lost: 2, points: 0, goalsFor: 1, goalsAgainst: 9, goalDifference: -8 }] }, { stage: "GROUP_STAGE", type: "TOTAL", group: "GROUP_G", table: [{ position: 1, team: { id: 25, name: "Egypt", tla: "EGY" }, playedGames: 2, form: null, won: 1, draw: 1, lost: 0, points: 4, goalsFor: 4, goalsAgainst: 2, goalDifference: 2 }, { position: 2, team: { id: 26, name: "Iran", tla: "IRN" }, playedGames: 2, form: null, won: 0, draw: 2, lost: 0, points: 2, goalsFor: 2, goalsAgainst: 2, goalDifference: 0 }, { position: 3, team: { id: 27, name: "Belgium", tla: "BEL" }, playedGames: 2, form: null, won: 0, draw: 2, lost: 0, points: 2, goalsFor: 1, goalsAgainst: 1, goalDifference: 0 }, { position: 4, team: { id: 28, name: "New Zealand", tla: "NZL" }, playedGames: 2, form: null, won: 0, draw: 1, lost: 1, points: 1, goalsFor: 3, goalsAgainst: 5, goalDifference: -2 }] }, { stage: "GROUP_STAGE", type: "TOTAL", group: "GROUP_H", table: [{ position: 1, team: { id: 29, name: "Spain", tla: "ESP" }, playedGames: 2, form: null, won: 1, draw: 1, lost: 0, points: 4, goalsFor: 4, goalsAgainst: 0, goalDifference: 4 }, { position: 2, team: { id: 30, name: "Uruguay", tla: "URU" }, playedGames: 2, form: null, won: 0, draw: 2, lost: 0, points: 2, goalsFor: 3, goalsAgainst: 3, goalDifference: 0 }, { position: 3, team: { id: 31, name: "Cape Verde", tla: "CPV" }, playedGames: 2, form: null, won: 0, draw: 2, lost: 0, points: 2, goalsFor: 2, goalsAgainst: 2, goalDifference: 0 }, { position: 4, team: { id: 32, name: "Saudi Arabia", tla: "KSA" }, playedGames: 2, form: null, won: 0, draw: 1, lost: 1, points: 1, goalsFor: 1, goalsAgainst: 5, goalDifference: -4 }] }, { stage: "GROUP_STAGE", type: "TOTAL", group: "GROUP_I", table: [{ position: 1, team: { id: 33, name: "France", tla: "FRA" }, playedGames: 2, form: null, won: 2, draw: 0, lost: 0, points: 6, goalsFor: 6, goalsAgainst: 1, goalDifference: 5 }, { position: 2, team: { id: 34, name: "Norway", tla: "NOR" }, playedGames: 2, form: null, won: 2, draw: 0, lost: 0, points: 6, goalsFor: 7, goalsAgainst: 3, goalDifference: 4 }, { position: 3, team: { id: 35, name: "Senegal", tla: "SEN" }, playedGames: 2, form: null, won: 0, draw: 0, lost: 2, points: 0, goalsFor: 3, goalsAgainst: 6, goalDifference: -3 }, { position: 4, team: { id: 36, name: "Iraq", tla: "IRQ" }, playedGames: 2, form: null, won: 0, draw: 0, lost: 2, points: 0, goalsFor: 1, goalsAgainst: 7, goalDifference: -6 }] }, { stage: "GROUP_STAGE", type: "TOTAL", group: "GROUP_J", table: [{ position: 1, team: { id: 37, name: "Argentina", tla: "ARG" }, playedGames: 2, form: null, won: 2, draw: 0, lost: 0, points: 6, goalsFor: 5, goalsAgainst: 0, goalDifference: 5 }, { position: 2, team: { id: 38, name: "Austria", tla: "AUT" }, playedGames: 2, form: null, won: 1, draw: 0, lost: 1, points: 3, goalsFor: 3, goalsAgainst: 3, goalDifference: 0 }, { position: 3, team: { id: 40, name: "Algeria", tla: "ALG" }, playedGames: 2, form: null, won: 1, draw: 0, lost: 1, points: 3, goalsFor: 2, goalsAgainst: 4, goalDifference: -2 }, { position: 4, team: { id: 39, name: "Jordan", tla: "JOR" }, playedGames: 2, form: null, won: 0, draw: 0, lost: 2, points: 0, goalsFor: 2, goalsAgainst: 5, goalDifference: -3 }] }, { stage: "GROUP_STAGE", type: "TOTAL", group: "GROUP_K", table: [{ position: 1, team: { id: 43, name: "Portugal", tla: "POR" }, playedGames: 2, form: null, won: 1, draw: 1, lost: 0, points: 4, goalsFor: 6, goalsAgainst: 1, goalDifference: 5 }, { position: 2, team: { id: 41, name: "Colombia", tla: "COL" }, playedGames: 1, form: null, won: 1, draw: 0, lost: 0, points: 3, goalsFor: 3, goalsAgainst: 1, goalDifference: 2 }, { position: 3, team: { id: 42, name: "DR Congo", tla: "COD" }, playedGames: 1, form: null, won: 0, draw: 1, lost: 0, points: 1, goalsFor: 1, goalsAgainst: 1, goalDifference: 0 }, { position: 4, team: { id: 44, name: "Uzbekistan", tla: "UZB" }, playedGames: 2, form: null, won: 0, draw: 0, lost: 2, points: 0, goalsFor: 1, goalsAgainst: 8, goalDifference: -7 }] }, { stage: "GROUP_STAGE", type: "TOTAL", group: "GROUP_L", table: [{ position: 1, team: { id: 45, name: "England", tla: "ENG" }, playedGames: 2, form: null, won: 1, draw: 1, lost: 0, points: 4, goalsFor: 4, goalsAgainst: 2, goalDifference: 2 }, { position: 2, team: { id: 46, name: "Ghana", tla: "GHA" }, playedGames: 2, form: null, won: 1, draw: 1, lost: 0, points: 4, goalsFor: 1, goalsAgainst: 0, goalDifference: 1 }, { position: 3, team: { id: 47, name: "Panama", tla: "PAN" }, playedGames: 1, form: null, won: 0, draw: 0, lost: 1, points: 0, goalsFor: 0, goalsAgainst: 1, goalDifference: -1 }, { position: 4, team: { id: 48, name: "Croatia", tla: "CRO" }, playedGames: 1, form: null, won: 0, draw: 0, lost: 1, points: 0, goalsFor: 2, goalsAgainst: 4, goalDifference: -2 }] }] }, scorers: { count: 18, competition: { id: 2e3, code: "WC", name: "FIFA World Cup" }, scorers: [{ player: { id: 9e3, name: "Erling Haaland", nationality: "Norway" }, team: { id: 34, name: "Norway", tla: "NOR" }, playedMatches: 2, goals: 4, assists: 0, penalties: 0 }, { player: { id: 9001, name: "Kylian Mbapp\xE9", nationality: "France" }, team: { id: 33, name: "France", tla: "FRA" }, playedMatches: 2, goals: 4, assists: 0, penalties: 1 }, { player: { id: 9002, name: "Harry Kane", nationality: "England" }, team: { id: 45, name: "England", tla: "ENG" }, playedMatches: 2, goals: 3, assists: 0, penalties: 1 }, { player: { id: 9003, name: "Florian Wirtz", nationality: "Germany" }, team: { id: 17, name: "Germany", tla: "GER" }, playedMatches: 2, goals: 3, assists: 0, penalties: 0 }, { player: { id: 9004, name: "Niclas F\xFCllkrug", nationality: "Germany" }, team: { id: 17, name: "Germany", tla: "GER" }, playedMatches: 2, goals: 3, assists: 0, penalties: 0 }, { player: { id: 9005, name: "Cody Gakpo", nationality: "Netherlands" }, team: { id: 21, name: "Netherlands", tla: "NED" }, playedMatches: 2, goals: 3, assists: 0, penalties: 0 }, { player: { id: 9006, name: "Lionel Messi", nationality: "Argentina" }, team: { id: 37, name: "Argentina", tla: "ARG" }, playedMatches: 2, goals: 3, assists: 0, penalties: 0 }, { player: { id: 9007, name: "Christian Pulisic", nationality: "United States" }, team: { id: 13, name: "United States", tla: "USA" }, playedMatches: 2, goals: 3, assists: 0, penalties: 1 }, { player: { id: 9008, name: "Takefusa Kubo", nationality: "Japan" }, team: { id: 22, name: "Japan", tla: "JPN" }, playedMatches: 2, goals: 2, assists: 0, penalties: 0 }, { player: { id: 9009, name: "Viktor Gy\xF6keres", nationality: "Sweden" }, team: { id: 23, name: "Sweden", tla: "SWE" }, playedMatches: 2, goals: 2, assists: 0, penalties: 0 }, { player: { id: 9010, name: "Mohamed Salah", nationality: "Egypt" }, team: { id: 25, name: "Egypt", tla: "EGY" }, playedMatches: 2, goals: 2, assists: 0, penalties: 1 }, { player: { id: 9011, name: "Vin\xEDcius J\xFAnior", nationality: "Brazil" }, team: { id: 9, name: "Brazil", tla: "BRA" }, playedMatches: 2, goals: 2, assists: 0, penalties: 0 }, { player: { id: 9012, name: "Juli\xE1n \xC1lvarez", nationality: "Argentina" }, team: { id: 37, name: "Argentina", tla: "ARG" }, playedMatches: 2, goals: 2, assists: 0, penalties: 0 }, { player: { id: 9013, name: "Folarin Balogun", nationality: "United States" }, team: { id: 13, name: "United States", tla: "USA" }, playedMatches: 2, goals: 2, assists: 0, penalties: 0 }, { player: { id: 9014, name: "Memphis Depay", nationality: "Netherlands" }, team: { id: 21, name: "Netherlands", tla: "NED" }, playedMatches: 2, goals: 2, assists: 0, penalties: 0 }, { player: { id: 9015, name: "Hakim Ziyech", nationality: "Morocco" }, team: { id: 10, name: "Morocco", tla: "MAR" }, playedMatches: 2, goals: 2, assists: 0, penalties: 1 }, { player: { id: 9016, name: "Du\u0161an Vlahovi\u0107", nationality: "Croatia" }, team: { id: 48, name: "Croatia", tla: "CRO" }, playedMatches: 2, goals: 1, assists: 0, penalties: 0 }, { player: { id: 9017, name: "Bruno Fernandes", nationality: "Portugal" }, team: { id: 43, name: "Portugal", tla: "POR" }, playedMatches: 2, goals: 1, assists: 0, penalties: 1 }] }, matches: { competition: { id: 2e3, code: "WC", name: "FIFA World Cup" }, resultSet: { count: 72 }, matches: [{ id: 1001, utcDate: "2026-06-13T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_A", homeTeam: { id: 1, name: "Mexico", tla: "MEX" }, awayTeam: { id: 4, name: "South Africa", tla: "RSA" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 2, away: 0 } }, goals: [] }, { id: 1002, utcDate: "2026-06-13T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_A", homeTeam: { id: 2, name: "South Korea", tla: "KOR" }, awayTeam: { id: 3, name: "Czechia", tla: "CZE" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 2, away: 1 } }, goals: [] }, { id: 1003, utcDate: "2026-06-19T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_A", homeTeam: { id: 3, name: "Czechia", tla: "CZE" }, awayTeam: { id: 4, name: "South Africa", tla: "RSA" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 1, away: 1 } }, goals: [] }, { id: 1004, utcDate: "2026-06-19T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_A", homeTeam: { id: 1, name: "Mexico", tla: "MEX" }, awayTeam: { id: 2, name: "South Korea", tla: "KOR" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 1, away: 0 } }, goals: [] }, { id: 1005, utcDate: "2026-06-13T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_B", homeTeam: { id: 6, name: "Switzerland", tla: "SUI" }, awayTeam: { id: 7, name: "Bosnia and Herzegovina", tla: "BIH" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 4, away: 1 } }, goals: [] }, { id: 1006, utcDate: "2026-06-13T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_B", homeTeam: { id: 5, name: "Canada", tla: "CAN" }, awayTeam: { id: 8, name: "Qatar", tla: "QAT" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 6, away: 0 } }, goals: [] }, { id: 1007, utcDate: "2026-06-19T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_B", homeTeam: { id: 5, name: "Canada", tla: "CAN" }, awayTeam: { id: 7, name: "Bosnia and Herzegovina", tla: "BIH" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 1, away: 1 } }, goals: [] }, { id: 1008, utcDate: "2026-06-19T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_B", homeTeam: { id: 8, name: "Qatar", tla: "QAT" }, awayTeam: { id: 6, name: "Switzerland", tla: "SUI" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 1, away: 1 } }, goals: [] }, { id: 1009, utcDate: "2026-06-14T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_C", homeTeam: { id: 9, name: "Brazil", tla: "BRA" }, awayTeam: { id: 10, name: "Morocco", tla: "MAR" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 1, away: 1 } }, goals: [] }, { id: 1010, utcDate: "2026-06-14T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_C", homeTeam: { id: 12, name: "Haiti", tla: "HAI" }, awayTeam: { id: 11, name: "Scotland", tla: "SCO" }, score: { winner: "AWAY_TEAM", duration: "REGULAR", fullTime: { home: 0, away: 1 } }, goals: [] }, { id: 1011, utcDate: "2026-06-20T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_C", homeTeam: { id: 11, name: "Scotland", tla: "SCO" }, awayTeam: { id: 10, name: "Morocco", tla: "MAR" }, score: { winner: "AWAY_TEAM", duration: "REGULAR", fullTime: { home: 0, away: 1 } }, goals: [] }, { id: 1012, utcDate: "2026-06-20T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_C", homeTeam: { id: 9, name: "Brazil", tla: "BRA" }, awayTeam: { id: 12, name: "Haiti", tla: "HAI" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 3, away: 0 } }, goals: [] }, { id: 1013, utcDate: "2026-06-12T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_D", homeTeam: { id: 13, name: "United States", tla: "USA" }, awayTeam: { id: 15, name: "Paraguay", tla: "PAR" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 4, away: 1 } }, goals: [] }, { id: 1014, utcDate: "2026-06-14T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_D", homeTeam: { id: 14, name: "Australia", tla: "AUS" }, awayTeam: { id: 16, name: "T\xFCrkiye", tla: "TUR" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 2, away: 0 } }, goals: [] }, { id: 1015, utcDate: "2026-06-18T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_D", homeTeam: { id: 13, name: "United States", tla: "USA" }, awayTeam: { id: 14, name: "Australia", tla: "AUS" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 2, away: 0 } }, goals: [] }, { id: 1016, utcDate: "2026-06-18T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_D", homeTeam: { id: 16, name: "T\xFCrkiye", tla: "TUR" }, awayTeam: { id: 15, name: "Paraguay", tla: "PAR" }, score: { winner: "AWAY_TEAM", duration: "REGULAR", fullTime: { home: 0, away: 1 } } }, { id: 1017, utcDate: "2026-06-15T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_E", homeTeam: { id: 17, name: "Germany", tla: "GER" }, awayTeam: { id: 20, name: "Cura\xE7ao", tla: "CUW" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 7, away: 1 } } }, { id: 1018, utcDate: "2026-06-15T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_E", homeTeam: { id: 18, name: "Ivory Coast", tla: "CIV" }, awayTeam: { id: 19, name: "Ecuador", tla: "ECU" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 1, away: 0 } } }, { id: 1019, utcDate: "2026-06-21T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_E", homeTeam: { id: 17, name: "Germany", tla: "GER" }, awayTeam: { id: 18, name: "Ivory Coast", tla: "CIV" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 2, away: 1 } } }, { id: 1020, utcDate: "2026-06-21T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_E", homeTeam: { id: 19, name: "Ecuador", tla: "ECU" }, awayTeam: { id: 20, name: "Cura\xE7ao", tla: "CUW" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 0, away: 0 } } }, { id: 1021, utcDate: "2026-06-15T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_F", homeTeam: { id: 21, name: "Netherlands", tla: "NED" }, awayTeam: { id: 22, name: "Japan", tla: "JPN" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 2, away: 2 } } }, { id: 1022, utcDate: "2026-06-15T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_F", homeTeam: { id: 23, name: "Sweden", tla: "SWE" }, awayTeam: { id: 24, name: "Tunisia", tla: "TUN" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 5, away: 1 } } }, { id: 1023, utcDate: "2026-06-21T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_F", homeTeam: { id: 21, name: "Netherlands", tla: "NED" }, awayTeam: { id: 23, name: "Sweden", tla: "SWE" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 5, away: 1 } } }, { id: 1024, utcDate: "2026-06-21T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_F", homeTeam: { id: 22, name: "Japan", tla: "JPN" }, awayTeam: { id: 24, name: "Tunisia", tla: "TUN" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 4, away: 0 } } }, { id: 1025, utcDate: "2026-06-16T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_G", homeTeam: { id: 27, name: "Belgium", tla: "BEL" }, awayTeam: { id: 25, name: "Egypt", tla: "EGY" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 1, away: 1 } } }, { id: 1026, utcDate: "2026-06-16T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_G", homeTeam: { id: 26, name: "Iran", tla: "IRN" }, awayTeam: { id: 28, name: "New Zealand", tla: "NZL" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 2, away: 2 } } }, { id: 1027, utcDate: "2026-06-22T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_G", homeTeam: { id: 27, name: "Belgium", tla: "BEL" }, awayTeam: { id: 26, name: "Iran", tla: "IRN" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 0, away: 0 } } }, { id: 1028, utcDate: "2026-06-22T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_G", homeTeam: { id: 28, name: "New Zealand", tla: "NZL" }, awayTeam: { id: 25, name: "Egypt", tla: "EGY" }, score: { winner: "AWAY_TEAM", duration: "REGULAR", fullTime: { home: 1, away: 3 } } }, { id: 1029, utcDate: "2026-06-16T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_H", homeTeam: { id: 29, name: "Spain", tla: "ESP" }, awayTeam: { id: 31, name: "Cape Verde", tla: "CPV" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 0, away: 0 } } }, { id: 1030, utcDate: "2026-06-16T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_H", homeTeam: { id: 32, name: "Saudi Arabia", tla: "KSA" }, awayTeam: { id: 30, name: "Uruguay", tla: "URU" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 1, away: 1 } } }, { id: 1031, utcDate: "2026-06-22T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_H", homeTeam: { id: 29, name: "Spain", tla: "ESP" }, awayTeam: { id: 32, name: "Saudi Arabia", tla: "KSA" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 4, away: 0 } } }, { id: 1032, utcDate: "2026-06-22T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_H", homeTeam: { id: 30, name: "Uruguay", tla: "URU" }, awayTeam: { id: 31, name: "Cape Verde", tla: "CPV" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 2, away: 2 } } }, { id: 1033, utcDate: "2026-06-17T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_I", homeTeam: { id: 33, name: "France", tla: "FRA" }, awayTeam: { id: 35, name: "Senegal", tla: "SEN" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 3, away: 1 } } }, { id: 1034, utcDate: "2026-06-17T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_I", homeTeam: { id: 36, name: "Iraq", tla: "IRQ" }, awayTeam: { id: 34, name: "Norway", tla: "NOR" }, score: { winner: "AWAY_TEAM", duration: "REGULAR", fullTime: { home: 1, away: 4 } } }, { id: 1035, utcDate: "2026-06-22T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_I", homeTeam: { id: 33, name: "France", tla: "FRA" }, awayTeam: { id: 36, name: "Iraq", tla: "IRQ" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 3, away: 0 } } }, { id: 1036, utcDate: "2026-06-23T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_I", homeTeam: { id: 34, name: "Norway", tla: "NOR" }, awayTeam: { id: 35, name: "Senegal", tla: "SEN" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 3, away: 2 } }, goals: [] }, { id: 1037, utcDate: "2026-06-17T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_J", homeTeam: { id: 37, name: "Argentina", tla: "ARG" }, awayTeam: { id: 40, name: "Algeria", tla: "ALG" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 3, away: 0 } } }, { id: 1038, utcDate: "2026-06-17T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_J", homeTeam: { id: 38, name: "Austria", tla: "AUT" }, awayTeam: { id: 39, name: "Jordan", tla: "JOR" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 3, away: 1 } } }, { id: 1039, utcDate: "2026-06-22T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_J", homeTeam: { id: 37, name: "Argentina", tla: "ARG" }, awayTeam: { id: 38, name: "Austria", tla: "AUT" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 2, away: 0 } } }, { id: 1040, utcDate: "2026-06-23T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_J", homeTeam: { id: 40, name: "Algeria", tla: "ALG" }, awayTeam: { id: 39, name: "Jordan", tla: "JOR" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 2, away: 1 } }, goals: [] }, { id: 1041, utcDate: "2026-06-17T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_K", homeTeam: { id: 41, name: "Colombia", tla: "COL" }, awayTeam: { id: 44, name: "Uzbekistan", tla: "UZB" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 3, away: 1 } } }, { id: 1042, utcDate: "2026-06-17T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_K", homeTeam: { id: 43, name: "Portugal", tla: "POR" }, awayTeam: { id: 42, name: "DR Congo", tla: "COD" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 1, away: 1 } } }, { id: 1043, utcDate: "2026-06-23T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_K", homeTeam: { id: 43, name: "Portugal", tla: "POR" }, awayTeam: { id: 44, name: "Uzbekistan", tla: "UZB" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 5, away: 0 } }, goals: [] }, { id: 1044, utcDate: "2026-06-17T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_L", homeTeam: { id: 45, name: "England", tla: "ENG" }, awayTeam: { id: 48, name: "Croatia", tla: "CRO" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 4, away: 2 } } }, { id: 1045, utcDate: "2026-06-17T12:00:00Z", status: "FINISHED", matchday: 1, stage: "GROUP_STAGE", group: "GROUP_L", homeTeam: { id: 46, name: "Ghana", tla: "GHA" }, awayTeam: { id: 47, name: "Panama", tla: "PAN" }, score: { winner: "HOME_TEAM", duration: "REGULAR", fullTime: { home: 1, away: 0 } } }, { id: 1046, utcDate: "2026-06-23T12:00:00Z", status: "FINISHED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_L", homeTeam: { id: 45, name: "England", tla: "ENG" }, awayTeam: { id: 46, name: "Ghana", tla: "GHA" }, score: { winner: "DRAW", duration: "REGULAR", fullTime: { home: 0, away: 0 } } }, { id: 1047, utcDate: "2026-06-23T12:00:00Z", status: "TIMED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_K", homeTeam: { id: 41, name: "Colombia", tla: "COL" }, awayTeam: { id: 42, name: "DR Congo", tla: "COD" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1048, utcDate: "2026-06-23T12:00:00Z", status: "TIMED", matchday: 2, stage: "GROUP_STAGE", group: "GROUP_L", homeTeam: { id: 47, name: "Panama", tla: "PAN" }, awayTeam: { id: 48, name: "Croatia", tla: "CRO" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1049, utcDate: "2026-06-24T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_A", homeTeam: { id: 1, name: "Mexico", tla: "MEX" }, awayTeam: { id: 3, name: "Czechia", tla: "CZE" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1050, utcDate: "2026-06-24T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_A", homeTeam: { id: 2, name: "South Korea", tla: "KOR" }, awayTeam: { id: 4, name: "South Africa", tla: "RSA" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1051, utcDate: "2026-06-24T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_B", homeTeam: { id: 5, name: "Canada", tla: "CAN" }, awayTeam: { id: 6, name: "Switzerland", tla: "SUI" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1052, utcDate: "2026-06-24T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_B", homeTeam: { id: 7, name: "Bosnia and Herzegovina", tla: "BIH" }, awayTeam: { id: 8, name: "Qatar", tla: "QAT" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1053, utcDate: "2026-06-25T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_C", homeTeam: { id: 9, name: "Brazil", tla: "BRA" }, awayTeam: { id: 11, name: "Scotland", tla: "SCO" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1054, utcDate: "2026-06-25T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_C", homeTeam: { id: 10, name: "Morocco", tla: "MAR" }, awayTeam: { id: 12, name: "Haiti", tla: "HAI" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1055, utcDate: "2026-06-25T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_D", homeTeam: { id: 13, name: "United States", tla: "USA" }, awayTeam: { id: 16, name: "T\xFCrkiye", tla: "TUR" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1056, utcDate: "2026-06-25T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_D", homeTeam: { id: 14, name: "Australia", tla: "AUS" }, awayTeam: { id: 15, name: "Paraguay", tla: "PAR" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1057, utcDate: "2026-06-26T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_E", homeTeam: { id: 17, name: "Germany", tla: "GER" }, awayTeam: { id: 19, name: "Ecuador", tla: "ECU" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1058, utcDate: "2026-06-26T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_E", homeTeam: { id: 18, name: "Ivory Coast", tla: "CIV" }, awayTeam: { id: 20, name: "Cura\xE7ao", tla: "CUW" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1059, utcDate: "2026-06-26T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_F", homeTeam: { id: 21, name: "Netherlands", tla: "NED" }, awayTeam: { id: 24, name: "Tunisia", tla: "TUN" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1060, utcDate: "2026-06-26T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_F", homeTeam: { id: 22, name: "Japan", tla: "JPN" }, awayTeam: { id: 23, name: "Sweden", tla: "SWE" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1061, utcDate: "2026-06-27T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_G", homeTeam: { id: 27, name: "Belgium", tla: "BEL" }, awayTeam: { id: 28, name: "New Zealand", tla: "NZL" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1062, utcDate: "2026-06-27T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_G", homeTeam: { id: 26, name: "Iran", tla: "IRN" }, awayTeam: { id: 25, name: "Egypt", tla: "EGY" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1063, utcDate: "2026-06-27T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_H", homeTeam: { id: 29, name: "Spain", tla: "ESP" }, awayTeam: { id: 30, name: "Uruguay", tla: "URU" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1064, utcDate: "2026-06-27T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_H", homeTeam: { id: 31, name: "Cape Verde", tla: "CPV" }, awayTeam: { id: 32, name: "Saudi Arabia", tla: "KSA" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1065, utcDate: "2026-06-28T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_I", homeTeam: { id: 33, name: "France", tla: "FRA" }, awayTeam: { id: 34, name: "Norway", tla: "NOR" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1066, utcDate: "2026-06-28T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_I", homeTeam: { id: 35, name: "Senegal", tla: "SEN" }, awayTeam: { id: 36, name: "Iraq", tla: "IRQ" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1067, utcDate: "2026-06-28T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_J", homeTeam: { id: 37, name: "Argentina", tla: "ARG" }, awayTeam: { id: 39, name: "Jordan", tla: "JOR" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1068, utcDate: "2026-06-28T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_J", homeTeam: { id: 38, name: "Austria", tla: "AUT" }, awayTeam: { id: 40, name: "Algeria", tla: "ALG" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1069, utcDate: "2026-06-29T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_K", homeTeam: { id: 41, name: "Colombia", tla: "COL" }, awayTeam: { id: 43, name: "Portugal", tla: "POR" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1070, utcDate: "2026-06-29T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_K", homeTeam: { id: 44, name: "Uzbekistan", tla: "UZB" }, awayTeam: { id: 42, name: "DR Congo", tla: "COD" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1071, utcDate: "2026-06-29T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_L", homeTeam: { id: 45, name: "England", tla: "ENG" }, awayTeam: { id: 47, name: "Panama", tla: "PAN" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }, { id: 1072, utcDate: "2026-06-29T12:00:00Z", status: "TIMED", matchday: 3, stage: "GROUP_STAGE", group: "GROUP_L", homeTeam: { id: 48, name: "Croatia", tla: "CRO" }, awayTeam: { id: 46, name: "Ghana", tla: "GHA" }, score: { winner: null, duration: "REGULAR", fullTime: { home: null, away: null } } }] } };
...
async function readWcData(sb, type) {
  const { data, error: error3 } = await sb.from("wc_data").select("data, source").eq("type", type).single();
  if (error3 || !data) return null;
  return { data: data.data, source: data.source };
}
__name(readWcData, "readWcData");
async function readWcMatches(sb) {
  const { data, error: error3 } = await sb.from("wc_matches").select("data").order("utc_date", { ascending: true });
  if (error3 || !data || data.length === 0) return null;
  const matches = data.map((r) => r.data);
  const { data: meta } = await sb.from("wc_sync_meta").select("source").eq("type", "matches").single();
  return { matches, source: meta?.source ?? "snapshot" };
}
__name(readWcMatches, "readWcMatches");
async function writeWcData(sb, type, data, source) {
  await Promise.all([
    sb.from("wc_data").upsert(
      { type, data, source, updated_at: (/* @__PURE__ */ new Date()).toISOString() },
      { onConflict: "type" }
    ),
    sb.from("wc_sync_meta").upsert(
      { type, last_sync_at: (/* @__PURE__ */ new Date()).toISOString(), source },
      { onConflict: "type" }
    )
  ]);
}
__name(writeWcData, "writeWcData");
async function writeWcMatches(sb, matches, source) {
  const rows = matches.map((m) => ({
    ...
    utc_date: m.utcDate ?? (/* @__PURE__ */ new Date()).toISOString(),
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  }));
  if (rows.length > 0) {
  ...
  await sb.from("wc_sync_meta").upsert(
    { type: "matches", last_sync_at: (/* @__PURE__ */ new Date()).toISOString(), source },
    { onConflict: "type" }
  );
}
__name(writeWcMatches, "writeWcMatches");

// lib/snapshot.ts
var SNAP = snapshot_default;
var FD_BASE = "https://api.football-data.org/v4/competitions/WC";
var PATHS = {
  standings: "/standings",
...
function fireAndForget(sb, type, data) {
  const source = "live";
  if (type === "matches" && data.matches) {
    writeWcMatches(sb, data.matches, source).catch(() => {
    });
  } else if (type !== "matches") {
    writeWcData(sb, type, data, source).catch(() => {
    });
  }
}
__name(fireAndForget, "fireAndForget");
async function getWcData(type, token, sbConfig) {
  if (token) {
    try {
      ...
      if (res.ok) {
        ...
        if (sbConfig) {
          try {
            const sb = createClient2(sbConfig.url, sbConfig.key);
    ...
    try {
      ...
      if (type === "matches") {
        ...
        if (result && result.matches.length > 0) {
          return { data: { matches: result.matches }, source: "supabase" };
        }
      } else {
        const result = await readWcData(sb, type);
        if (result) {
  ...
  return { data: fallback, source: "snapshot" };
}
__name(getWcData, "getWcData");
function toJson2(data, source, maxAge) {
  const cacheControl = maxAge != null ? `public, max-age=${maxAge}` : source === "live" ? "no-store, no-cache, must-revalidate" : source === "supabase" ? "public, max-age=120, s-maxage=120" : "public, max-age=3600";
  return new Response(
    JSON.stringify({
      ...data,
      _source: source,
      _asOf: source === "snapshot" ? SNAP._meta.asOf ?? SNAP._meta.asof : (/* @__PURE__ */ new Date()).toISOString()
    }),
    {
      headers: {
        "Content-Type": "application/json",
...
// api/wc/[type].ts
var VALID_TYPES = /* @__PURE__ */ new Set(["standings", "scorers", "matches", "teams"]);
var onRequest6 = /* @__PURE__ */ __name(async (ctx) => {
  ...
  const sbConfig = ctx.env.SUPABASE_URL && ctx.env.SUPABASE_SERVICE_ROLE_KEY ? { url: ctx.env.SUPABASE_URL, key: ctx.env.SUPABASE_SERVICE_ROLE_KEY } : void 0;
  const { data, source } = await getWcData(type, token, sbConfig);
  if (!data) return new Response(JSON.stringify({ error: "No data" }), { status: 502 });
  return toJson2(data, source, type === "teams" ? 21600 : void 0);
}, "onRequest");

// api/sync.ts
var FD_BASE2 = "https://api.football-data.org/v4/competitions/WC";
function checkSecret(ctx) {
  const url = new URL(ctx.request.url);
...
__name(checkSecret, "checkSecret");
var onRequestPost = /* @__PURE__ */ __name(async (ctx) => {
  ...
  const sb = createClient2(sbUrl, sbKey);
  const results = {};
  const endpoints = [
    { type: "matches", path: "/matches?dateFrom=2026-06-10&dateTo=2026-07-20" },
    { type: "standings", path: "/standings" },
    { type: "scorers", path: "/scorers?limit=30" },
    { type: "teams", path: "/teams" }
  ];
  for (const { type, path } of endpoints) {
    try {
      const res = await fetch(FD_BASE2 + path, {
        headers: { "X-Auth-Token": token }
      });
      if (!res.ok) {
      ...
      if (type === "matches" && data.matches) {
        ...
        const count3 = Array.isArray(data[type]) ? data[type].length : 0;
        results[type] = `${count3} ${type} synced`;
      }
    } catch (e) {
      results[type] = `Error: ${e.message}`;
    }
  }
  return new Response(JSON.stringify({ ok: true, results, syncedAt: (/* @__PURE__ */ new Date()).toISOString() }), {
...
// ../.wrangler/tmp/pages-bmxrDk/functionsRoutes-0.9459446868540543.mjs
var routes = [
  {
