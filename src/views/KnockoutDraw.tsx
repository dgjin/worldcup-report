...
function SvgBracket({ byStage }: { byStage: Map<KnockoutStage, MatchRaw[]> }) {
  ...
  const centerY = cy(3, 0);

  const sideMatches = (stage: KnockoutStage, perSide: number) => {
    const list = byStage.get(stage) ?? [];
    return { left: list.slice(0, perSide), right: list.slice(perSide, perSide * 2) };
  };
  ...
  for (let round = 0; round <= 3; round++) {
    const { left, right } = sideMatches(ROUND_STAGES[round], PER_SIDE[round]);
    for (let idx = 0; idx < PER_SIDE[round]; idx++) {
      boxes.push(box(`L${round}-${idx}`, leftX(round), cy(round, idx) - BH / 2, left[idx] ?? null));
  ...
  for (let round = 1; round <= 3; round++) {
    for (let idx = 0; idx < PER_SIDE[round]; idx++) {
      const c1 = cy(round - 1, idx * 2);
      const c2 = cy(round - 1, idx * 2 + 1);
      const pcy = cy(round, idx);
      const lcx = leftX(round - 1) + BW;
      const lpx = leftX(round);
      const lm = (lcx + lpx) / 2;
      ...
      const rcx = rightX(round - 1);
      const rpx = rightX(round) + BW;
      const rm = (rcx + rpx) / 2;
  ...
  return (
    <div className="hidden overflow-x-auto pb-2 lg:block">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ minWidth: 1000 }} role="img" aria-label="淘汰赛对阵图">
        ...
        {box("FINAL", finalX, centerY - BH / 2, finalMatch, true)}
      </svg>
    </div>
  );
}
